...
def do_upload():
    ...
    infile = os.path.join(dirname, tif_filename)
    dstfile = os.path.join(dirname, asc_filename)

    if (tif2csv.tif2csv(filename=infile, dstfile=dstfile, xoff=xll, yoff=yll, width=width, height=height)):
        return static_file(asc_filename, root=dirname, download=asc_filename)
    else:
        raise bottle.HTTPerror(500, "Unable to convert file!")
